...
import {
  type IMigrationFileEntity,
  LocalMigrationEntity,
  type LocalMigrationRepository,
  RemoteMigrationEntity,
  type RemoteMigrationRepository,
    ...
    describe('migrations', () => {
      ...
      describe('pending migration execution', () => {
        ...
        it('should save applied migrations state to remote repository', async () => {
          ...
          expect(migrationService.appliedMigrations.every((m) => m.persisted)).toBe(
            true,
          )
        })
