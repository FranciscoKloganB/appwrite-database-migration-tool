...
export class RemoteMigrationRepository implements IMigrationRepository {
  ...
  public async insertMigration(
    migration: CreateMigrationEntity,
  ...
  public async listMigrations() {
    ...
    const entities = response.documents.map((document) => {
      ...
      throw new TypeError(
        `Unexpected document shape found in migration document ${document.$id}`,
      )
    })

    this.#log(
      `Remote entities retrieved: ${JSON.stringify(entities.map((x) => x.name))}`,
    )
  ...
  public async updateMigration(
    migration: UpdateMigrationEntity,
  ): Promise<RemoteMigrationEntity> {
