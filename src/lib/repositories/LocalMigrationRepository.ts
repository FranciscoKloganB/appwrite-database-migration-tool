...
export class LocalMigrationRepository implements IMigrationRepository {
  ...
  async deleteMigration(_migration: DeleteMigrationEntity): Promise<boolean> {
    throw new Error('Method not implemented.')
  }

  async insertMigration(
    _migration: CreateMigrationEntity,
  ...
  async listMigrations() {
    ...
    const entities = modules
      ...
      .map((module) => {
        ...
        this.#log(`Loaded local migration entity: ${name}`)
    ...
    this.#log(
      `Local entities retrieved: ${JSON.stringify(entities.map((x) => x.name))}`,
    )
  ...
  async updateMigration(
    _migration: UpdateMigrationEntity,
  ): Promise<LocalMigrationEntity> {
    throw new Error('Method not implemented.')
  }
  ...
  private async getFiles(dir: string, extensions: string[]): Promise<string[]> {
    ...
    const files = dirents
      .filter(
        (dent) =>
          dent.isFile() && extensions.includes(dent.name.split('.').pop() || ''),
      )
